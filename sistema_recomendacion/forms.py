...
class PonderacionesEquipoForm(forms.Form):
    ponderacion_pilotos = forms.FloatField(
        label='...que el equipo haya contado con alguno de tus pilotos favoritos? Dale un valor de 0 a 5',
        min_value=0, max_value=5, initial=3,        
        widget=forms.NumberInput(attrs={'step': '1'})
    )
    ponderacion_pais = forms.FloatField(
        label='...que el país del equipo coincida con el país de tus equipos favoritos? Dale un valor de 0 a 5',
        min_value=0, max_value=5, initial=4,
        widget=forms.NumberInput(attrs={'step': '1'})
    )
    ponderacion_longevidad = forms.FloatField(
        label='...la longevidad del equipo en Fórmula 1? Dale un valor de 0 a 5',
        min_value=0, max_value=5, initial=2,
        widget=forms.NumberInput(attrs={'step': '1'})
    )
    ponderacion_mundiales = forms.FloatField(
